...
> **地图页是完全离线的**：不加载任何外部 CSS/JS/瓦片（本机访问不到 unpkg 与
> `tile.openstreetmap.org`，用 Leaflet+OSM 的话页面会全黑）。
> 页面用 Canvas + Web 墨卡托投影自己画：经纬网格 + 轨迹 + 当前点 + 比例尺，
> 支持拖拽平移、滚轮缩放、双击回到跟随。**没有外网也能正常看。**
...
- **电脑端**：内存环形队列最多 5000 点，超出丢最旧。**离线自绘地图**（Canvas + Web 墨卡托）：
  经纬网格（间隔随缩放自动调整）+ 轨迹 polyline + 起点标记 + 当前点脉冲圈与速度/电量文字
  + 比例尺；自动贴合轨迹范围，支持拖拽平移 / 滚轮缩放 / 双击回到跟随。
