...
CHANNEL_ADB = "adb"
CHANNEL_WIFI = "wifi"
CHANNEL_CLOUDFLARE = "cloudflare"  # Cloudflare 隧道：手机通过公网地址直连电脑（本机回环转发）
CHANNEL_PRIORITY = {CHANNEL_ADB: 3, CHANNEL_WIFI: 2, CHANNEL_CLOUDFLARE: 1, CHANNEL_NONE: 0}
...
class ConnectionManager(QObject):
    ...
    def _save_settings_cache(self, secret_token=None):
        """保存设置到缓存和文件"""
    ...
    def _setup_flask_routes(self):
        ...
        def _update_phone_connection():
            ...
            if not remote_ip:
                return
            # Cloudflare 隧道：手机通过公网地址直连，经 cloudflared 本机转发后 remote_addr 为 127.0.0.1，
            # 用手机端上报的 X-Channel 头区分（与 ADB 的 127.0.0.1 不混淆）
            phone_channel = request.headers.get('X-Channel', '').lower()
            self.last_phone_seen = time.time()
            if phone_channel == CHANNEL_CLOUDFLARE:
                if not self.phone_connected or self.current_channel != CHANNEL_CLOUDFLARE:
                    self._set_channel(CHANNEL_CLOUDFLARE)
                return
    ...
    def _probe_channels(self):
        ...
        while self.is_running:
            ...
            # WiFi / Cloudflare 通道无需主动探测（通道由手机主动发起连接）
    ...
    def _reconnect(self):
        ...
        for _ in range(self.RECONNECT_RETRY):
            if channel == CHANNEL_ADB and self._check_adb():
                return True
            elif channel in (CHANNEL_WIFI, CHANNEL_CLOUDFLARE):
                # WiFi / Cloudflare 通道无法主动探测手机（手机无回连 HTTP 服务），
                # 循环内重试无意义，直接进入降级判断，避免一直 return True 导致"已连接"状态卡死
                break
    ...
    def _send_to_phone(self, msg):
        ...
        # 所有通道（WiFi/ADB/Cloudflare）均为手机轮询 /api/poll 主动拉取，统一走内存队列
        with self.queue_lock:
    ...
    def send_file(self, file_path):
        ...
        if self.current_channel in (CHANNEL_WIFI, CHANNEL_CLOUDFLARE):
            # WiFi / Cloudflare 通道：手机通过 /api/download_file 轮询拉取
            pass
    ...
    def _file_transfer_watchdog(self, file_id):
        ...
        while self.outgoing_file_id == file_id and self.file_transfer_active:
            ...
            if getattr(self, '_transfer_paused', False):
                no_progress_seconds = 0
                continue
            # last_transfer_progress_time == 0 表示手机尚未开始下载（用户还未点"开始下载"），
            # 等待用户确认期间不计入超时，否则用户稍后点击会因状态被清空而 404
            if self.last_transfer_progress_time > 0 and time.time() - self.last_transfer_progress_time > 5:
    ...
    def _pc_audio_loop(self):
        """电脑音频捕获循环：优先 WASAPI loopback 捕获系统输出声音，找不到回退麦克风。
        采集到的 PCM 分块写入有界队列，由 /api/audio 路由 drain 给手机，保证连续推流（不再单块覆盖）。
        """
        self._pc_audio_source = "unknown"
        try:
            import pyaudio
            p = pyaudio.PyAudio()

            # 定位 WASAPI 主机（loopback 设备只在 WASAPI 下可靠捕获系统声音）
            try:
                for hi in range(p.get_host_api_count()):
                    hi_info = p.get_host_api_info_by_index(hi)
                    if 'wasapi' in str(hi_info.get('name', '')).lower():
                        break
            except Exception:
                pass
            ...
            default_output_name = ''
            try:
                default_output_name = p.get_default_output_device_info().get('name', '')
            except Exception:
                pass

            # 候选回环设备：优先与默认输出设备同名，确保抓的是当前扬声器在放的声音
            loopback_candidates = []
            for i in range(p.get_device_count()):
                try:
                    ...
                    name = d.get('name', '')
                    low = name.lower()
                    is_loop = ('loopback' in low or '循环' in name
                               or 'stereo mix' in low or '立体声混音' in name
                               or 'what u hear' in low)
                    if is_loop:
                        if default_output_name and default_output_name in name:
                            loopback_candidates.insert(0, i)
                        else:
            ...
            if loopback_candidates:
                ...
                self.log(f"[audio] 使用系统回环设备捕获电脑声音: index={chosen}")
            else:
                # 没找到回环设备：回退到默认输入（麦克风），并明确提示用户可能无声
                try:
                    chosen = p.get_default_input_device_info()['index']
                    self._pc_audio_source = "mic"
                    self.log("[audio] 未找到系统回环(loopback)设备，已回退到麦克风输入。"
                             "若手机听不到电脑声音，请开启 Windows『立体声混音』或安装虚拟音频线(VB-Cable)。")
                except Exception:
                    chosen = None

            if chosen is None:
                self._pc_audio_source = "none"
                self.log("[audio] 无任何可用音频输入设备，声音传输中止")
                self._pc_audio_running = False
                return
            ...
            try:
                stream = p.open(format=FORMAT, channels=CHANNELS, rate=RATE,
                                input=True, input_device_index=chosen,
                                frames_per_buffer=CHUNK)
            except Exception as e:
                # 指定设备打开失败，再尝试默认参数
                self.log(f"[audio] 以指定设备打开失败({e})，尝试默认输入设备")
                try:
                    stream = p.open(format=FORMAT, channels=CHANNELS, rate=RATE,
                ...
                try:
                    ...
                    if data:
                        ...
                        if silent_streak == 200:
                            self.log("[audio] 提示：已连续约 9 秒静音。请确认电脑正在播放声音，"
                                     "或回环设备选错（抓到了麦克风/无信号源）。")
                            silent_streak = 0
                except Exception as e:
                    if any(k in str(e).lower() for k in ("stream", "device", "winerror", "portaudio")):
                        self.log(f"[audio] 音频捕获中断: {e}")
                        break
                    time.sleep(0.01)
            try:
                stream.stop_stream()
            ...
            self._pc_audio_source = "none"
            self.log("pyaudio未安装，无法进行声音传输。请运行: pip install pyaudio")
            self._pc_audio_running = False
        except Exception as e:
            self._pc_audio_source = "none"
